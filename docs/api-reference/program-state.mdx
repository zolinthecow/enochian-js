...
[`.assistant`](/api-reference/program-state#user-assitant-system), [`.system`](/api-reference/program-state#user-assitant-system), and [`.gen`](/api-reference/program-state#gen). The idea is that the `ProgramState` should only be affected
...
but should never change any of it's values.
...
Gets the value that was generated for a given `answerKey` that was passed into [`.gen`](/api-reference/program-state#gen). Returns
