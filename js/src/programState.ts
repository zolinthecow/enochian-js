...
type GenAsyncFunctionResultWithSchema<K extends string, S> = {
  genFunction: GenAsyncFunctionReturnType;
  _schemaType: S;
  key: K;
};
type GenAsyncGeneratorResultWithSchema<K extends string, S> = {
  genFunction: GenAsyncGeneratorReturnType;
  _schemaType: S;
  key: K;
};
type GenResultWithSchema<K extends string, S> =
  | GenAsyncFunctionResultWithSchema<K, S>
  | GenAsyncGeneratorResultWithSchema<K, S>;

type MessageWithSchema<
  Gens extends readonly (
    | GenResultWithSchema<string, unknown>
    | Stringifiable
...
export default class ProgramState<
  T extends Record<string, { val: unknown; resp: GenerateRespSingle }>,
> {
  ...
  async fromSGL(opts: SGLSetModelParams): Promise<ProgramState<T>> {
    console.log("HI", opts);
  ...
  private _createRoleFunction(role: "user" | "assistant" | "system") {
    ...
    function roleFunction<
      const G extends readonly (
        | GenAsyncFunctionResultWithSchema<string, unknown>
        | Promise<Stringifiable>
        | Stringifiable
      )[],
    >(
      ...
      strings: TemplateStringsArray,
      ...values: [...G]
    ): Promise<MessageWithSchema<G>>;
    function roleFunction<
      const G extends readonly (
        | GenAsyncGeneratorResultWithSchema<string, unknown>
        | GenAsyncFunctionResultWithSchema<string, unknown>
        | Promise<Stringifiable>
        | Stringifiable
      )[],
    >(
      ...
      strings: TemplateStringsArray,
      ...values: [...G]
    ): AsyncGenerator<MessageWithSchema<G>>;
    function roleFunction<
      const G extends readonly (
        | GenAsyncGeneratorResultWithSchema<string, unknown>
        | GenAsyncFunctionResultWithSchema<string, unknown>
    ...
    >(
      ...
      if (values.some((v) => isGenAsyncGenerator(v))) {
        return async function* (this: ProgramState<T>) {
  ...
  private _processRoleStringTemplate(
    ...
    ...values:
      | Stringifiable[]
      | (GenAsyncFunctionReturnType | Promise<Stringifiable> | Stringifiable)[]
      | (
          | GenAsyncGeneratorReturnType
          | GenAsyncFunctionReturnType
          | Promise<Stringifiable>
        ...
        for (let i = 0; i < strings.length; i++) {
          ...
          if (i < values.length) {
            ...
            if (isGenAsyncGenerator(value)) {
              const generator = value([...this._messages, curMessage]);
  ...
  gen<const K extends string, S extends z.ZodType = z.ZodString>(
    answerKey: K,
    genInput?: Omit<GenerateReqNonStreamingInput<S>, "text" | "input_ids">,
  ): (messages: Message[]) => Promise<string>;
  gen<const K extends string, S extends z.ZodType = z.ZodString>(
    answerKey: K,
    genInput?: Omit<GenerateReqStreamingInput<S>, "text" | "input_ids">,
  ): (messages: Message[]) => AsyncGenerator<string, void, unknown>;
  gen<const K extends string, S extends z.ZodType = z.ZodString>(
    answerKey: K,
    genInput?:
      | Omit<GenerateReqNonStreamingInput<S>, "text" | "input_ids">
      | Omit<GenerateReqStreamingInput<S>, "text" | "input_ids">,
  ):
    | ((messages: Message[]) => Promise<string>)
    | ((messages: Message[]) => AsyncGenerator<string, void, unknown>) {
    ...
    if (!genInput || isNonStreamingInput(genInput)) {
      return async (messages: Message[]): Promise<string> => {
        ...
        this._answers[answerKey] = {
          val: genInput?.sampling_params?.zod_schema
            ? genInput.sampling_params.zod_schema.parse(JSON.parse(ans.text))
            : ans.text,
          resp: ans,
        };
        return ans.text;
      ...
      return async function* (messages: Message[]) {
        ...
        let chunk: IteratorResult<GenerateRespSingle> = await generator.next();
        while (!chunk.done) {
